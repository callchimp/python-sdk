...
class CallReportRequest(BaseModel):
    ...
    CallReportRequest
    """ # noqa: E501
    var_from: StrictInt = Field(description="Unix timestamp in milliseconds, start of range.", alias="from")
    to: StrictInt = Field(description="Unix timestamp in milliseconds, end of range.")
    campaign: Optional[StrictInt] = Field(default=None, description="Campaign foreign key, only use when report is required for a specific campaign.")
    __properties: ClassVar[List[str]] = ["from", "to", "campaign"]
