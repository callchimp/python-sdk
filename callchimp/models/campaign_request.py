...
class CampaignRequest(BaseModel):
    ...
    type: StrictStr = Field(description="Type of campaign, we have four types Outbound Bulk (blastout), Outbound AI (outbound), Inbound AI (inbound), Transactional (transactional)")
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['blastout', 'outbound', 'inbound', 'transactional']):
            raise ValueError("must be one of enum values ('blastout', 'outbound', 'inbound', 'transactional')")
        return value
